...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('industry.update', $industry->id) }}" class="form-horizontal" method="POST">
                        @csrf
                        @method('PATCH')
                        @include('economy.industry._form', ['buttonText' => "परिवर्तन गर्नुहोस्", 'industry' => $industry])
                    </form>
                </div>
            </div>
        </div>
